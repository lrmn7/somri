...
// ✨ 1. Perbanyak daftar gambar kartu Anda hingga 20
const cardImages = [
  { id: 1, name: "1.png" },
  { id: 2, name: "2.png" },
  { id: 3, name: "3.png" },
...
export function GameBoard() {
  ...
  const { isSuccess: hasPaid } = useWaitForTransactionReceipt({
    hash: payHash,
  });
  const { isSuccess: isSubmitted } = useWaitForTransactionReceipt({
    hash: scoreHash,
  });
      ...
      {gamePhase === "won" && (
        <div className="text-center p-6 bg-gray-800 rounded-lg shadow-xl mt-4">
          ...
          <p className="text-xl mb-4">
            Your final score:{" "}
            <span className="font-bold text-yellow-300">{finalScore}</span>
          </p>{" "}
          <button
            onClick={() =>
              submitScore({
            ...
            className="bg-purple-600 hover:bg-purple-700 disabled:bg-gray-500 font-bold py-2 px-6 rounded-lg"
          >
            {" "}
            {isSubmitting
              ? "Submitting..."
