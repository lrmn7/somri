...
export function AdminPanel() {
  ...
  const { data: minScore, refetch: refetchMinScore } = useReadContract({
    ...contractConfig,
  ...
  const { data: easyReward, refetch: refetchEasyReward } = useReadContract({
    ...contractConfig,
    functionName: "rewardAmounts",
    args: [Difficulty.Easy],
  });
  const { data: mediumReward, refetch: refetchMediumReward } = useReadContract({
    ...contractConfig,
    functionName: "rewardAmounts",
    args: [Difficulty.Medium],
  });
  const { data: hardReward, refetch: refetchHardReward } = useReadContract({
    ...contractConfig,
    functionName: "rewardAmounts",
  ...
  const handleSetPlayFee = () => {
    if (!newPlayFee || parseFloat(newPlayFee) < 0)
      return toast.error("Invalid fee");
    writeContract({
      ...contractConfig,
      functionName: "setPlayFee",
      args: [parseEther(newPlayFee)],
    });
  };
  const handleSetRewardAmount = () => {
    if (!newRewardAmount || parseFloat(newRewardAmount) < 0)
      return toast.error("Invalid reward");
    writeContract({
      ...contractConfig,
      functionName: "setRewardAmount",
      args: [selectedDifficulty, parseEther(newRewardAmount)],
    });
  };
  const handleSetMinScore = () => {
    ...
    writeContract({
      ...contractConfig,
      functionName: "setMinScore",
      args: [BigInt(newMinScore)],
    });
  };
  const handleDeposit = () => {
    if (!depositValue || parseFloat(depositValue) <= 0)
      return toast.error("Invalid deposit");
    writeContract({
      ...contractConfig,
  ...
  const handleWithdraw = () => {
    if (!withdrawValue || parseFloat(withdrawValue) <= 0)
      return toast.error("Invalid withdraw");
    writeContract({
      ...contractConfig,
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
        <div className="bg-gray-700 p-4 rounded-lg">
          ...
          <p className="text-xl font-bold">
            {contractBalance
              ? `${parseFloat(contractBalance.formatted).toFixed(4)} STT`
              : "..."}
          </p>
        </div>
        <div className="bg-gray-700 p-4 rounded-lg">
          <p className="text-sm text-gray-400">Play Fee</p>
          <p className="text-xl font-bold">
            {playFee !== undefined
              ? `${formatEther(playFee as bigint)} STT`
              : "..."}
          </p>
        </div>
        <div className="bg-gray-700 p-4 rounded-lg">
          ...
          <p className="text-xl font-bold">
            {minScore !== undefined ? (minScore as bigint).toString() : "..."}
          </p>
        </div>
        <div className="bg-green-900/50 p-4 rounded-lg">
          <p className="text-sm text-green-300">Easy Reward</p>
          <p className="text-xl font-bold">
            {easyReward !== undefined
              ? `${formatEther(easyReward as bigint)} STT`
              : "..."}
          </p>
        </div>
        <div className="bg-yellow-900/50 p-4 rounded-lg">
          <p className="text-sm text-yellow-300">Medium Reward</p>
          <p className="text-xl font-bold">
            {mediumReward !== undefined
              ? `${formatEther(mediumReward as bigint)} STT`
              : "..."}
          </p>
        </div>
        <div className="bg-red-900/50 p-4 rounded-lg">
          <p className="text-sm text-red-300">Hard Reward</p>
          <p className="text-xl font-bold">
            {hardReward !== undefined
              ? `${formatEther(hardReward as bigint)} STT`
              : "..."}
          </p>
        </div>
      </div>
        ...
        <div className="space-y-4 bg-gray-900/50 p-4 rounded-lg">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="New Min Score"
              className="input-field"
            />
            <button
              onClick={handleSetMinScore}
              disabled={isPending}
              className="btn-primary w-48"
            >
              {isPending ? "Pending..." : "Set Min Score"}
            </button>
          </div>
        </div>
        <div className="space-y-4 bg-gray-900/50 p-4 rounded-lg">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Amount to Deposit (STT)"
              className="input-field"
            />
            <button
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Amount to Withdraw (STT)"
              className="input-field"
            />
            <button
